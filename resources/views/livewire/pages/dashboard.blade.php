            ...
            <div class="max-w-7xl space-y-5 mx-auto sm:px-6 lg:px-8">
                ...
                <div class="flex w-full gap-4 mb-8">
                    <section class="bg-white w-64  overflow-hidden shadow-md p-4 sm:rounded-lg">
                        ...
                        <div>
                            <h3 class="text-2xl font-bold underline">{{auth()->user()->kandang->nama_kandang ?? '-'}}</h3>
                        </div>
                    </section>
                    <section class="bg-white w-72 flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                        ...
                        <div class="flex justify-between items-center gap-x-5">
                            <span class="text-4xl font-extrabold">{{ number_format($totalAyam, 0, ',', '.') }} <span class="text-gray-500 text-sm">Ekor</span></span>
                            <div class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full inline-flex items-center font-bold px-1 py-0.2 gap-1">
                                <span class="text-sm"> ↓ {{$ayamMatiMinggu }}%</span>
                            </div>
                        </div>
                        <span class="text-sm mt-10 italic text-gray-500 float-end">Akhir minggu ini</span>
                    </section>
                    <section class="bg-white w-72 h-max flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                        ...
                        <div class="flex flex-col flex-end gap-x-5">
                            <div class="flex justify-between items-center gap-x-5">
                                <span class="text-4xl font-extrabold">{{number_format($hasil['jumlah'], 0, ',', '.') }} <span class="text-sm text-gray-500">Butir</span></span>
                                <div class="{{ $hasil['naik'] ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }} text-xs font-medium me-2 px-2.5 py-0.5 rounded-full inline-flex items-center font-bold px-1 py-0.2 gap-1">
                                    {{ $hasil['naik'] ? '↑' : '↓' }} {{$hasil['persentase'] }} % 
                                </div>
                            </div>
                            <span class="text-sm mt-10 italic text-gray-500 float-end">Akhir minggu ini</span>
                        </div>
                ...
                <div class="flex w-full gap-x-10 mb-8">
                ...
                <div class="w-full flex flex-col">
                    <div class="flex w-full gap-4 mb-8">
                        <section class="bg-white w-72  overflow-hidden shadow-md p-4 sm:rounded-lg">
                            ...
                            <div class="flex justify-between items-center gap-x-5">
                                <span class="text-4xl font-extrabold">{{ $totalEmployees }}</span></span>
                            </div>
                        </section>
                        ...
                        <section class="bg-white w-72  overflow-hidden shadow-md p-4 sm:rounded-lg">
                            ...
                            <div class="flex justify-between items-center gap-x-5">
                                <span class="text-4xl font-extrabold">{{ $totalChickenCoops }}</span>
                            </div>
                        </section>
                        ...
                        <section class="bg-white w-72 flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                            ...
                            <span class="text-sm mt-4 italic text-gray-500 float-end">Bulan Ini</span>
                        </section>
                        ...
                        <section class="bg-white w-72 h-max flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                            ...
                            <div class="flex flex-col flex-end gap-x-5">
                                <div class="flex justify-between items-center gap-x-5">
                                    <span class="text-4xl font-extrabold">{{ $deadChickens }} <span class="text-sm text-gray-500">Ekor</span></span>
                                    {{-- <div class="{{ $hasil['naik'] ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }} text-xs font-medium me-2 px-2.5 py-0.5 rounded-full inline-flex items-center font-bold px-1 py-0.2 gap-1">
                                        {{ $hasil['naik'] ? '↑' : '↓' }} {{$hasil['persentase'] }} % 
                                    </div> --}}
                                </div>
                            ...
                            @push('scripts')
                                <script>
                                    var options = {
                                        ...
                                        series:[{{ $liveChickens }}, {{ $deadChickens }}],
                                        chart: {
                                            type: 'donut',
                                            height: 600,
                                        },
                    ...
                    <div class="flex w-full justify-between gap-x-5 mb-8">
                        <div class="w-2/3">
                        ...
                        {{-- table --}}
                        <section class="bg-white w-1/3 overflow-hidden shadow-md p-4 sm:rounded-lg">
                            <div class="flex-col flex gap-y-2 text-gray-500 mb-3">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="size-10 bg-gray-200 p-2 rounded-md">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0 1 12 15a9.065 9.065 0 0 0-6.23-.693L5 14.5m14.8.8 1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0 1 12 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" />
                                    </svg>
                            ...
                            <div class="flex flex-col gap-y-2">
                                <article>
                                    <span class="text-sm text-gray-500 font-bold">Jagung</span>
                                    <div class="flex w-full bg-gray-100 rounded-full h-2.5">
                                        <div class="bg-yellow-300 h-2.5 rounded-full w-full inline-flex items-center" style="width: {{$feed['jagung']}}%"><span class="text-sm text-gray-900 p-2">{{$feed['jagung']}}%</span></div>
                                    </div>
                                </article>
                                <article>
                                    <span class="text-sm text-gray-500 font-bold">Multivitamin</span>
                                    <div class="flex w-full bg-gray-100 rounded-full h-2.5">
                                        <div class="bg-violet-300 h-2.5 rounded-full w-full inline-flex items-center" style="width: {{$feed['multivitamin']}}%"><span class="text-sm text-gray-900 p-2">{{$feed['multivitamin']}}%</span></div>
                                    </div>
                                </article>
                            </div>
                        </section>
