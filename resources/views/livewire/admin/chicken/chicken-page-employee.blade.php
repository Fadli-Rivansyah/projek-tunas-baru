            ...
            <div class="w-full flex  gap-x-5 mb-8 fade-up">
                ...
                <section class="bg-white w-64 flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                    <div class="flex-col flex gap-y-2 text-gray-500 mb-3">
                        ...
                        <span class="font-bold">Ayam Mati</span>
                    </div>
                    <div class="flex justify-between items-center gap-x-5 mb-4">
                        <span class="text-4xl font-extrabold">{{ $montlyDeadChickens }}<span class="text-gray-500 text-sm"> Ekor</span></span>
                    </div>
                ...
                <section class="bg-white w-64 flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                    <div class="flex-col flex gap-y-2 text-gray-500 mb-4">
                        ...
                        <span class="font-bold">Jumlah Ayam</span>
                    </div>
                    <div class="flex items-center gap-x-5 mb-4">
                        <span class="text-4xl font-extrabold">{{ $chickenCount }}<span class="text-gray-500 text-sm"> Ekor</span></span>
                    </div>
                    <span class="w-max bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full  border border-green-400">Dikelolah</span>
                </section>
                {{-- card age chicken --}}
                <section class="bg-white w-64 flex justify-between flex-col overflow-hidden shadow-md p-4 sm:rounded-lg">
                    <div class="flex-col flex gap-y-2 text-gray-500 mb-4">
                    ...
                    <div class="flex items-center gap-x-5 mb-4 ">
                        <span class="text-4xl font-extrabold">{{ $chickenAge }}<span class="text-gray-500 text-sm"> Minggu</span></span>
                    </div>
                    <span class="w-max bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full  border border-green-400">Diawal</span>
                </section>
            </div>
                ...
                <div>
                    ...
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-3 mb-4 fade-up">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
                            ...
                            <tbody>
                                @forelse ($this->tableChickens['montlyChickens'] as $item)
                                    <tr class="odd:bg-white  border-b  border-gray-200">
                                        ...
                                        <td scope="row" class="px-6 py-4">
                                            {{ number_format($item->total_ayam, 0, ',', '.') ?? 0  }} Ekor
                                        </td>
                                        <td class="px-6 py-4">
                                            {{  number_format($item->jumlah_ayam_mati, 0, ',', '.') ?? 0 }} Ekor
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ number_format($item->jumlah_pakan , 0, ',', '.') ?? 0 }} Kg
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                    ...
                    {{-- section pagination --}}
                    <div>
                        {{ $this->tableChickens['montlyChickens']->links() }}
                    </div>
                </div>
                 <section class="bg-white w-1/3 h-max shadow-md p-4 sm:rounded-lg">
                    ...
                    @push('scripts')
                        <script>
                            var options = {
                                title: {
                                    text: 'Detail Ayam Keseluruhan',
                                    align: 'left'
                                },
                                labels: ['Ayam Hidup', 'Ayam Mati', 'Usia Ayam'],
                                series:[ {{ $this->chartChickens['liveChicken'] }}, {{ $this->chartChickens['deadChicken'] }}, {{ $this->chartChickens['ageChickenNow'] }}],
                                chart: {
                                    type: 'donut',
                                    height: 600,
                                },
